...
export class Cell {
    ...
    setMass(mass) { this.mass = mass; this.recalculateRadius(); }
    addMass(mass) { this.setMass(this.mass + mass); }
...
export class PlayerLogic {
    constructor(id) {
        this.skinId = "default";
        this.customBodyColor = null; // free RGB e.g. "#ff4400"
        this.customShotColor = null;
        this.name = null;
        this.cells = [];
        this.massTotal = 0;
        this.x = 0;
    ...
    loseMassIfNeeded(massLossRate, defaultPlayerMass, minMassLoss) {
        if (this.massTotal > minMassLoss && Date.now() - this.lastMassLossTime > 2000) {
            ...
            for (let i = 1; i < this.cells.length; i++) {
                if (this.cells[i].mass > this.cells[maxIndex].mass)
                    maxIndex = i;
            }
    ...
    virusSplit(cellIndex, limitSplit, defaultPlayerMass) {
    ...
    move(slowBase, gameWidth, gameHeight, initMassLog) {
        ...
        for (let i = 0; i < this.cells.length; i++) {
            ...
            if (cell.x < clampOffset)
                cell.x = clampOffset;
            if (cell.x > gameWidth - clampOffset)
                cell.x = gameWidth - clampOffset;
            if (cell.y < clampOffset)
                cell.y = clampOffset;
            if (cell.y > gameHeight - clampOffset)
                cell.y = gameHeight - clampOffset;
        }

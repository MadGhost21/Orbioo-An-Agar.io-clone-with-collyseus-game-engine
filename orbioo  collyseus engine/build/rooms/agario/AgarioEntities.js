...
const EJECT_FRICTION = 0.85;
const EJECT_MIN_SPEED = 0.5;
...
export class EjectedMass {
    constructor(position, mass, color, speed, direction, ownerId) {
        this.id = uuidv4();
        this.x = position.x;
        this.y = position.y;
        this.mass = mass;
    ...
    isStopped() { return this.speed < EJECT_MIN_SPEED; }
}
export class EntityManager {
    ...
    addEjectedMass(cellX, cellY, cellRadius, targetX, targetY, playerX, playerY, mass, color, ownerId, gameWidth, gameHeight) {
        ...
        this.massFood.push(new EjectedMass({ x: safeX, y: safeY }, mass, color, EJECT_SPEED, direction, ownerId));
        return direction;
    }
    ...
    moveEjectedMass(gameWidth, gameHeight) {
        for (const ball of this.massFood)
            ball.move(gameWidth, gameHeight);
    }
    moveViruses(gameWidth, gameHeight) {
        for (const virus of this.viruses)
    ...
    balanceMass() {
        ...
        if (virusesToAdd > 0) {
            for (let i = 0; i < virusesToAdd; i++) {
                const mass = AgarioUtils.randomInRange(AgarioConfig.virus.defaultMass.from, AgarioConfig.virus.defaultMass.to);

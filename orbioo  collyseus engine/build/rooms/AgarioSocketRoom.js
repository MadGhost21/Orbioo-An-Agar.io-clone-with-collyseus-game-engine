...
__decorate([
    type("number")
], CellSchema.prototype, "x", void 0);
__decorate([
    type("number")
], CellSchema.prototype, "y", void 0);
__decorate([
    type("number")
], CellSchema.prototype, "mass", void 0);
__decorate([
    type("number")
], CellSchema.prototype, "radius", void 0);
export class PlayerSchema extends Schema {
    constructor() {
        super(...arguments);
        this.name = "Unnamed";
        this.score = 0; // mass-based score
        this.hue = 0; // base hue if default skin
        this.customColor = ""; // override rgb string if custom
        this.skinId = "default";
        this.spawnShield = false; // V1.4.2
        this.x = 0; // V1.4.2.1 Anchor for AOI
        this.y = 0; // V1.4.2.1 Anchor for AOI
        this.isAFK = false; // V1.4.8
        this.afkTime = 0; // V1.4.8
...
__decorate([
    type("number")
], PlayerSchema.prototype, "hue", void 0);
__decorate([
    type("string")
], PlayerSchema.prototype, "customColor", void 0);
__decorate([
    type("string")
], PlayerSchema.prototype, "skinId", void 0);
__decorate([
    type("boolean")
], PlayerSchema.prototype, "spawnShield", void 0);
__decorate([
    type("number")
...
__decorate([
    type("boolean")
], PlayerSchema.prototype, "isAFK", void 0);
__decorate([
    type("number")
], PlayerSchema.prototype, "afkTime", void 0);
__decorate([
    type([CellSchema])
], PlayerSchema.prototype, "cells", void 0);
export class FoodSchema extends Schema {
}
__decorate([
    type("string")
], FoodSchema.prototype, "id", void 0);
__decorate([
    type("number")
], FoodSchema.prototype, "x", void 0);
__decorate([
    type("number")
], FoodSchema.prototype, "y", void 0);
__decorate([
    type("number")
], FoodSchema.prototype, "radius", void 0);
__decorate([
    type("number")
], FoodSchema.prototype, "hue", void 0);
export class MassFoodSchema extends Schema {
}
__decorate([
    type("string")
], MassFoodSchema.prototype, "id", void 0);
__decorate([
    type("number")
], MassFoodSchema.prototype, "x", void 0);
__decorate([
    type("number")
], MassFoodSchema.prototype, "y", void 0);
__decorate([
    type("number")
], MassFoodSchema.prototype, "radius", void 0);
__decorate([
    type("string")
], MassFoodSchema.prototype, "color", void 0);
...
__decorate([
    type("string")
], VirusSchema.prototype, "id", void 0);
__decorate([
    type("number")
], VirusSchema.prototype, "x", void 0);
__decorate([
    type("number")
], VirusSchema.prototype, "y", void 0);
__decorate([
    type("number")
], VirusSchema.prototype, "radius", void 0);
__decorate([
    type("boolean")
], VirusSchema.prototype, "isMoving", void 0);
export class GridSectorSchema extends Schema {
    constructor() {
        super(...arguments);
        this.d = ""; // Base64-encoded binary mask: [x,y,h][x,y,h]... (V1.4.5)
    }
}
__decorate([
    type("string")
], GridSectorSchema.prototype, "d", void 0);
// ── AOI helper: compute centroid of a player's cells ────────────────────────
function playerCentroid(player) {
    if (!player)
        return null;
...
export class AgarioState extends Schema {
    constructor() {
        ...
        this.massFood = new MapSchema();
        this.viruses = new MapSchema();
    }
}
__decorate([
    type("number")
], AgarioState.prototype, "gameWidth", void 0);
__decorate([
    type("number")
], AgarioState.prototype, "gameHeight", void 0);
__decorate([
    type("number")
], AgarioState.prototype, "foodCount", void 0);
__decorate([
    type("number")
], AgarioState.prototype, "virusCount", void 0);
__decorate([
    type({ map: PlayerSchema })
], AgarioState.prototype, "players", void 0);
__decorate([
    view(),
    type({ map: GridSectorSchema })
], AgarioState.prototype, "grid", void 0);
__decorate([
    view(),
    type({ map: MassFoodSchema })
], AgarioState.prototype, "massFood", void 0);
__decorate([
    view(),
    type({ map: VirusSchema })
], AgarioState.prototype, "viruses", void 0);
// ── Colour helpers ────────────────────────────────────────
function hslStr(h, s = 100, l = 50) { return `hsl(${h},${s}%,${l}%)`; }
// ── Room Implementation ───────────────────────────────────
export class AgarioSocketRoom extends Room {
    constructor() {
        super(...arguments);
        this.maxClients = 50;
        this.playerLogics = new Map();
        this.entities = new EntityManager();
        this.initMassLog = AgarioUtils.mathLog(AgarioConfig.defaultPlayerMass, AgarioConfig.slowBase);
        this._leaderboardTick = 0;
        this._foodBySector = new Map();
        this._foodHash = new SpatialHash(250);
        this._massHash = new SpatialHash(250);
        this._virusHash = new SpatialHash(250);
        this._lastFoodCount = -1;
        this._lastMassCount = -1;
        this._lastVirusCount = -1;
    }
    onCreate(options) {
        ...
        this.onMessage("respawn", (client) => {
            ...
            if (!schema || !logic)
                return;
            logic.init(AgarioUtils.randomPosition(AgarioUtils.massToRadius(AgarioConfig.defaultPlayerMass)), AgarioConfig.defaultPlayerMass);
        ...
        this.onMessage("1", (client) => {
            ...
            for (let i = 0; i < logic.cells.length; i++) {
                ...
                const direction = this.entities.addEjectedMass(cell.x, cell.y, cell.radius, logic.target.x, logic.target.y, logic.x, logic.y, AgarioConfig.fireFood, shotColor, client.sessionId, AgarioConfig.gameWidth, AgarioConfig.gameHeight);
                // Check if this ball immediately hits a virus
                const justAdded = this.entities.massFood[this.entities.massFood.length - 1];
                if (justAdded)
                    this.entities.feedVirusIfHit(justAdded);
            }
        });
        // ── Split ────────────────────────────────────────
        ...
        // ── Skin / color ──────────────────────────────────
        this.onMessage("setSkin", (client, payload) => {
            const logic = this.playerLogics.get(client.sessionId);
            const schema = this.state.players.get(client.sessionId);
            if (!logic || !schema)
                return;
            if (typeof payload === "string") {
                if (ALL_SKINS.includes(payload)) {
    ...
    onJoin(client, options) {
        const name = (options?.name ?? "Unnamed").toString().substring(0, 25);
        console.log(`[Worker ${process.env.PORT}] 🟢 Player Joined: ${name} (${client.sessionId})`);
        const logic = new PlayerLogic(client.sessionId);
        logic.name = name;
        // ... (existing logic continues)
        const skinIdFromOptions = options?.skinId ?? "default";
        logic.skinId = ALL_SKINS.includes(skinIdFromOptions) ? skinIdFromOptions : "default";
    ...
    onLeave(client, _code) {
        ...
        if (logic)
            this.broadcast("playerDisconnect", { name: logic.name ?? "" });
    }
    // ── Update tick ───────────────────────────────────────
    update(dt) {
        // V1.4.9.7: Skip heavy logic if room is empty to save CPU
        if (this.clients.length === 0 && this.playerLogics.size === 0)
            return;
        try {
            this._updateCore(dt);
        }
    ...
    handleCollisions() {
        ...
        const playerCellHash = new SpatialHash(250);
        ...
        this.playerLogics.forEach((playerA, idA) => {
            for (let indexA = playerA.cells.length - 1; indexA >= 0; indexA--) {
                ...
                for (const ball of nearbyMass) {
                    if (ball.ownerId === idA && (now - ball.createdAt) < EJECT_OWNER_DELAY)
                        continue;
                ...
                for (const virus of nearbyViruses) {
                    ...
                    if (!SAT.testCircleCircle(circleA, vc, resp))
                        continue;
                ...
                for (const entry of nearbyOtherCells) {
                    ...
                    if (playerA.spawnShield || (playerB && playerB.spawnShield))
                        continue;
                    ...
                    if (whoAteWho === 1 && this.playerLogics.get(idB)?.name?.startsWith("Bot_"))
                        whoAteWho = 0;
                    if (whoAteWho === -1 && playerA.name?.startsWith("Bot_"))
                        whoAteWho = 0;
                    if (whoAteWho === 1) {
                        const playerB = this.playerLogics.get(idB);
                        if (playerB) {
    ...
    _updateAOIViews() {
        this.clients.forEach(client => {
            ...
            const prevVisibleKeys = client.viewedSectors || new Set();
            ...
            client.viewedSectors = currentVisibleKeys;
            ...
            const prevMassIds = client.viewedMass || new Set();
            visibleMass.forEach(m => {
                const schema = this.state.massFood.get(m.id);
                if (schema)
                    view.add(schema);
            });
            prevMassIds.forEach((id) => {
            ...
            client.viewedMass = currentMassIds;
            // Virus AOI
            ...
            const prevVirusIds = client.viewedViruses || new Set();
            visibleViruses.forEach(v => {
                const schema = this.state.viruses.get(v.id);
                if (schema)
                    view.add(schema);
            });
            prevVirusIds.forEach((id) => {
            ...
            client.viewedViruses = currentVirusIds;
        });
    }
    // ── Sync helpers ──────────────────────────────────────
    _refreshSector(key) {
                ...
                if (currentCount < localMax) {
                    ...
                    if (toAdd > 0) {
                        for (let i = 0; i < toAdd; i++) {
                            ...
                            if (!this._foodBySector.has(key))
                                this._foodBySector.set(key, new Set());
    ...
    _syncLogicToSchema(logic, schema) {
        ...
        while (schema.cells.length > logic.cells.length)
            schema.cells.pop();
    }
    _syncMassFoodToState() {
        const entityIds = new Set(this.entities.massFood.map(m => m.id));
        this.state.massFood.forEach((m, id) => { if (!entityIds.has(id))
            this.state.massFood.delete(id); });
        this.entities.massFood.forEach((m) => {
    ...
    _syncVirusesToState() {
        ...
        this.state.viruses.forEach((v, id) => { if (!entityIds.has(id))
            this.state.viruses.delete(id); });
    ...
    _broadcastLeaderboard() {
        ...
        this.clients.forEach(client => {
            ...
            if (myIdx === -1)
                return;
            const payload = { leaderboard: top10 };

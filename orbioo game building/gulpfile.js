...
function runServer(done) {
    nodemon({
        ...
        watch: ['./bin/server/'],
        ignore: ['./bin/server/db/'], // V1.5.4: Stop CMD flicker on Join
        ext: 'js html css',
        done
    })
}
...
function copyClientResources() {
    return gulp.src(['src/client/**/*.*', '!src/client/js/app.js'])
...
gulp.task('watch', gulp.series(setDev, 'dev', gulp.parallel(runServer, (done) => {
    gulp.watch(['src/server/**/*.*', 'src/server/**/*.js'], buildServer);
    gulp.watch(['src/client/**/*.*', 'src/client/js/app.js'], gulp.parallel(copyClientResources, buildClientJS));
})));

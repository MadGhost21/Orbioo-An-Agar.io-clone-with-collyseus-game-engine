...
const addPlayer = (socket) => {
    ...
    socket.on('gotit', function (clientPlayerData) {
        ...
        if (map.players.findIndexByID(socket.id) > -1) {
            ...
            // Critical fix: Send clean welcome message after joining as player
            socket.emit('welcome', {
                id: currentPlayer.id,
                ...
                x: currentPlayer.x,
                y: currentPlayer.y,
                cells: currentPlayer.cells.map(c => ({ x: c.x, y: c.y, mass: c.mass, radius: c.radius }))
            }, {
    ...
    socket.on('0', (target) => {
        ...
        // Target is a relative offset from client, always update it
        if (target) {
...
const tickGame = () => {
    ...
    map.players.handleCollisions(function (gotEaten, eater) {
        ...
        if (playerDied) {
            ...
            io.emit('playerDied', { playerEatenName: playerGotEaten.name }); // Match client expectation
...
const sendUpdates = () => {
    ...
    map.enumerateWhatPlayersSee(function (playerData, visiblePlayers, visibleFood, visibleMass, visibleViruses) {
        const binaryUpdate = binary.encodeUpdate(playerData, visiblePlayers, visibleFood, visibleMass, visibleViruses);
        sockets[playerData.id].emit('serverTellPlayerMove', binaryUpdate);
...
const updateSpectator = (socketID) => {
    ...
    const binaryUpdate = binary.encodeUpdate(playerData, map.players.data, map.food.data, map.massFood.data, map.viruses.data);
    sockets[socketID].emit('serverTellPlayerMove', binaryUpdate);
    if (leaderboardChanged) {
